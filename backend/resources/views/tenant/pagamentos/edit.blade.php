...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2>Editar Pagamento #{{ $pagamento->id }} - Fatura #{{ $fatura->numero }}</h2>
    ...
    <div class="card shadow-sm p-4">
        <form action="{{ route('tenant.pagamentos.update', [$fatura->id, $pagamento->id]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control @error('data_pagamento') is-invalid @enderror"
                       ...
                       value="{{ old('data_pagamento', $pagamento->data_pagamento) }}" required>
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control @error('valor_pago') is-invalid @enderror"
                       ...
                       placeholder="Ex: 1500,00" value="{{ old('valor_pago', $pagamento->valor_pago) }}" required>
                @error('valor_pago')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control @error('valor_troco') is-invalid @enderror"
                       ...
                       placeholder="0,00" value="{{ old('valor_troco', $pagamento->valor_troco) }}">
                @error('valor_troco')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control @error('valor_total_desconto') is-invalid @enderror"
                       name="valor_total_desconto" id="valor_total_desconto" 
                       placeholder="0,00" value="{{ old('valor_total_desconto', $pagamento->valor_total_desconto) }}">
            ...
            <div class="mb-4">
                ...
                <select class="form-select @error('metodo_pagamento') is-invalid @enderror" 
                    ...
                    <option value="cartão" {{ old('metodo_pagamento', $pagamento->metodo_pagamento)=='cartão' ? 'selected' : '' }}>Cartão</option>
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-save"></i> Atualizar Pagamento
                </button>
                <a href="{{ route('tenant.pagamentos.index', $fatura->id) }}" class="btn btn-secondary">
